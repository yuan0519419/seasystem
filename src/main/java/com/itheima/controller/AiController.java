...
public class AiController {
    ...
    public Map<String, Object> chat(@RequestBody Map<String, String> request) {
        ...
        try {
            ...
            // 构建请求URL（使用聊天API端点）
            URL url = new URL("https://dashscope.aliyuncs.com/v1/chat/completions");
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if (contentStart != -1) {
                    ...
                    if (contentEnd != -1) {
                        ...
                        response.put("message", "Failed to parse response: content end not found");
                    }
                } else {
                    response.put("data", null);
                    response.put("code", 500);
                    response.put("message", "Failed to parse response: content start not found");
                }
            } else {
                // 读取错误响应
    ...
    public Map<String, Object> getSuggestion() {
        Map<String, Object> response = new HashMap<>();
        try {
            AiSuggestion suggestion = aiSuggestionService.getSuggestion();
            response.put("code", 0);
            ...
            e.printStackTrace();
            response.put("code", 500);
            response.put("message", "获取AI研判建议失败");
            response.put("data", null);
        }
        return response;
    }
